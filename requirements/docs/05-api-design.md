...
POST   /api/sources/:sourceId/:mediaId/charactors       # 特定のメディアにキャラクターを追加します。(sourceId: UUID, mediaId: UUID, body: { charactorId: number })
DELETE /api/sources/:sourceId/:mediaId/charactors/:charactorId # 特定のメディアからキャラクターを削除します。(sourceId: UUID, mediaId: UUID, charactorId: number)
...
#### バルク操作
```
POST   /api/sources/:sourceId/media/bulk-edit   # 複数メディアを一括編集します。(sourceId: UUID)
DELETE /api/sources/:sourceId/media/bulk-delete # 複数メディアを一括削除します。(sourceId: UUID)
POST   /api/sources/:sourceId/media/bulk-move   # 複数メディアを一括移動します。(sourceId: UUID)
POST   /api/sources/:sourceId/media/bulk-tag    # 複数メディアを一括タグ付けします。(sourceId: UUID)
```
