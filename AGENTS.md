...
-   **型安全性:**
    -   **`any` の禁止:** `any` 型の使用は厳禁です。Biomeのlinterルール (`noExplicitAny`) によりエラーとして検出されます。どうしても必要な場合は、理由をコメントに残した上で `// biome-ignore` を使用してください。
    -   **`as` (型アサーション) の禁止:** `as Type` による型アサーションは禁止します（`as const` は除く）。型アサーションはコンパイラの型チェックを無効化し、ランタイムエラーの原因となります。代わりに Type Guard 関数や Zod によるバリデーションを使用してください。
    -   **Deprecated API の回避:** 非推奨（Deprecated）のAPIやライブラリの使用を避けてください。エディタ上の警告（取り消し線など）に注意を払い、代替のAPIを使用してください。
    -   **Import Type:** 型定義のインポートには `import type` を使用します。
-   **インポートエイリアス:**
