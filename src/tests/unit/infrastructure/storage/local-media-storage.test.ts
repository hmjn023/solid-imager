...
describe("LocalMediaStorage Unit Tests", () => {
    ...
    beforeEach(() => {
        ...
        const mockSharpInstance = {
            metadata: vi.fn().mockResolvedValue({ width: EXPECTED_WIDTH, height: 600 }),
        };
        // @ts-ignore - Mocking default export function behavior
        ...
        // @ts-ignore - Mocking library structure
    ...
    describe("copyFile", () => {
        it("should succeed for video files using ffmpeg", async () => {
            ...
            // Execute
            const result = await LocalMediaStorage.copyFile(sourcePath, targetPath, {});
