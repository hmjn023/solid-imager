...
test.describe("Media Sources Management", () => {
  ...
  test.describe("Source Creation", () => {
    test("should open add source modal when clicking Add Source button", async ({ page }) => {
      // Click the Add Source button
      await page.click('button:has-text("Add Source")');

      // Verify modal is opened
      await expect(page.locator('h2:has-text("Add Media Source")')).toBeVisible();

      // Verify form fields are present
      await expect(page.locator('input[placeholder="Enter source name"]')).toBeVisible();
      await expect(page.locator('input[placeholder="Enter description (optional)"]')).toBeVisible();
      await expect(page.locator('select')).toBeVisible();
      await expect(page.locator('input[placeholder="Enter file path"]')).toBeVisible();
    });
    ...
    test("should create a new local source successfully", async ({ page }) => {
      ...
      // Fill form
      await page.fill('input[placeholder="Enter source name"]', "Test Local Source");
      await page.fill('input[placeholder="Enter description (optional)"]', "Test description");
      ...
      await expect(page.locator('h2:has-text("Add Media Source")')).not.toBeVisible();
    ...
    test("should create a new SFTP source successfully", async ({ page }) => {
      ...
      await page.fill('input[placeholder="Enter source name"]', "Test SFTP Source");
      await page.fill('input[placeholder="Enter description (optional)"]', "SFTP test description");
      await page.selectOption('select', "sftp");
      await page.fill('input[placeholder="Enter file path"]', "/sftp/test/path");
    ...
    test("should create a new S3 source successfully", async ({ page }) => {
      ...
      await page.fill('input[placeholder="Enter source name"]', "Test S3 Source");
      await page.fill('input[placeholder="Enter description (optional)"]', "S3 test description");
      await page.selectOption('select', "s3");
      await page.fill('input[placeholder="Enter file path"]', "s3://test-bucket/path");
  ...
  test.describe("Form Validation", () => {
    ...
    test("should work with only required fields (name and path)", async ({ page }) => {
      await page.click('button:has-text("Add Source")');

      // Fill only required fields
      await page.fill('input[placeholder="Enter source name"]', "Minimal Source");
  ...
  test.describe("Source Editing", () => {
    test("should open edit modal with pre-filled data when clicking Edit", async ({ page }) => {
      // Wait for sources to load and click edit on first source
      await page.waitForSelector('[data-testid="source-card"]', { timeout: 5000 }).catch(() => {
      ...
      await expect(page.locator('h2:has-text("Edit Media Source")')).toBeVisible();
    ...
    test("should update source successfully", async ({ page }) => {
      ...
      // Modify the name
      await page.fill('input[placeholder="Enter source name"]', "Updated Source Name");
      await page.fill('input[placeholder="Enter description (optional)"]', "Updated description");

      // Submit update
      await page.click('button:has-text("Update")');

      // Verify modal closes
      await expect(page.locator('h2:has-text("Edit Media Source")')).not.toBeVisible();
  ...
  test.describe("Source Deletion", () => {
    test("should open delete confirmation modal when clicking Delete", async ({ page }) => {
      ...
      await expect(page.locator('h2:has-text("Delete Media Source")')).toBeVisible();
    ...
    test("should delete source when confirming", async ({ page }) => {
      ...
      const firstSourceName = await page.locator('[data-testid="source-card"] h3, .font-bold').first().textContent();
      ...
      // Verify modal closes
      await expect(page.locator('h2:has-text("Delete Media Source")')).not.toBeVisible();
    ...
    test("should cancel deletion when clicking Cancel", async ({ page }) => {
      const initialSourceCount = await page.locator('[data-testid="source-card"], .grid > div').count();
      ...
      await expect(page.locator('[data-testid="source-card"], .grid > div')).toHaveCount(initialSourceCount);
    });
  });
    ...
    test("should show loading state when submitting form", async ({ page }) => {
      ...
      await page.route('**/api/sources', async (route) => {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({ id: 'test', name: 'Loading Test', type: 'local' })
        });
      });
