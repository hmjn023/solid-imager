 ...
 * @property {string} mediaSourceId - The ID of the media source it belongs to.
 ...
 * @param {string} mediaSourceId - The ID of the media source to select from.
 ...
 */
export const selectRandomMedia = async (mediaSourceId: string): Promise<Media> => {
  try {
    const result = await db
      .select()
      .from(medias)
      .where(sql`${medias.mediaSourceId} = ${mediaSourceId}`)
      .orderBy(sql`RANDOM()`)
      .limit(1);

    if (result.length === 0) {
      throw new NotFoundError({
        message: "No random media found for the given mediaSourceId",
      });
    }
    return result[0];
  } catch (error) {
