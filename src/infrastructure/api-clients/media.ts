...
/**
 * Retrieves a specific media item by its media source ID and media ID.
...
export async function getMedia(
  ...
  const validatedMediaSourceId = mediaSourceIdSchema.parse(mediaSourceId);
  const validatedMediaId = mediaIdSchema.parse(mediaId);
  const result = await selectMediaById(validatedMediaId);

  if (result.mediaSourceId !== validatedMediaSourceId) {
...
export async function updateMedia(
  ...
  if (existingMedia.mediaSourceId !== validatedMediaSourceId) {
    throw new Error("Media not found");
  }
...
export async function deleteMedia(
  ...
  // サムネイルも非同期で削除します。
  try {
    await deleteThumbnail(validatedMediaSourceId, validatedMediaId);
  } catch (_error) {
...
export async function registerExistingMedia(
  ...
  for (const fullPath of mediaFiles) {
    try {
      ...
      const newMedia: NewMedia = {
        mediaSourceId: validatedMediaSourceId,
  ...
  if (addedMedia.length > 0) {
    ...
    addJobsToQueue(validatedMediaSourceId, jobs);
    startJobQueue(validatedMediaSourceId, async (job) => {
      const media = addedMedia.find((m) => m.id === job.mediaId);
      if (media) {
        await generateThumbnail(media, job.sourcePath, validatedMediaSourceId);
      }
    });
  }
...
export async function uploadMedia(
  ...
  const validatedMediaSourceId = mediaSourceIdSchema.parse(mediaSourceId);
  const mediaSource = await selectMediaSourceById(validatedMediaSourceId);
  ...
  addJobsToQueue(validatedMediaSourceId, [
...
export async function getAllMedia(mediaSourceId: string): Promise<Media[]> {
  const validatedMediaSourceId = mediaSourceIdSchema.parse(mediaSourceId);
  const result = await selectMediaBySourceId(validatedMediaSourceId);
  return result;
}
