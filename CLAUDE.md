...
- **No `as unknown as DomainModel`.** Every repository must expose a `mapToX(dbRow): X` function that explicitly converts DB rows (snake_case, nullable) to domain types (camelCase, `undefined`). Server and Tauri implementations share the *mapping logic* even though the repository shape differs (class vs. object literal).
