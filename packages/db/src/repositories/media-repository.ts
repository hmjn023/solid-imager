...
function makeBuildKeywordCondition(
...
function makeBuildRelationQuery(
	getExecutor: (tx?: unknown) => DrizzleExecutor,
) {
	return (
			...
			case "character":
				subquery = exists(
					getExecutor()
						...
						.innerJoin(
							characters,
							eq(mediaCharacters.characterId, characters.id),
						)
						.where(
							and(
...
function makeBuildGenerationInfoQuery(
	getExecutor: (tx?: unknown) => DrizzleExecutor,
) {
	return (
		_target: string,
...
function makeBuildCriterionQuery(
	getExecutor: (tx?: unknown) => DrizzleExecutor,
) {
	...
	return (node: SearchCriterion): SQL | undefined => {
		...
		if (target === "folder") {
			...
			const folderPath = node.value.endsWith("/")
				? node.value
				: `${node.value}/`;
...
function makeExecuteSearch(getExecutor: (tx?: unknown) => DrizzleExecutor) {
	return async (
		...
		const needsDetailsJoin = ["rating", "viewCount"].includes(
			params.sort ?? "",
		);
	...
	return {
		...
		async bulkRemoveTags(
			...
			try {
				...
				throw new UnexpectedError(
					"Failed to bulk remove tags from media",
					error,
				);
			}
		},
