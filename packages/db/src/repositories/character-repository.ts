...
import { characterIps, characters, ips, mediaCharacters } from "../schema";
...
export function createCharacterRepository(
  ...
  return {
    ...
    async create(character: NewCharacter, tx?: unknown): Promise<Character> {
      try {
        ...
        if (ipIds && ipIds.length > 0) {
          ipsResult = await client.select().from(ips).where(inArray(ips.id, ipIds));
        }
    ...
    async update(id: string, character: UpdateCharacter, tx?: unknown): Promise<Character> {
      try {
        const operation = async (client: DrizzleExecutor) => {
          ...
          if (!updatedData) {
            ...
            const existing = await this.findById(id, tx ?? client);
            if (!existing) {
        ...
        return await getExecutor().transaction((innerTx) => operation(innerTx));
      } catch (error) {
    ...
    async updateIpsBulk(
      ...
      try {
        ...
        await client.delete(characterIps).where(inArray(characterIps.characterId, characterIds));
    ...
    async findOrCreateBulk(
      ...
      const client = getExecutor(tx);
      const names = [
        ...new Set(charactersData.map((c) => c.name.trim()).filter((n) => n.length > 0)),
      ];
