...
import { charactersQueryKeys, ipsQueryKeys, projectsQueryKeys } from "./query-options";

export type MediaSidebarContentProps = {
  media: MediaDetails;
  isUpdating?: boolean;
  onUpdate?: () => void;
  aiTaggingModal: (props: { isOpen: boolean; onClose: () => void }) => JSX.Element;

  characterCropModal?: (props: { isOpen: boolean; onClose: () => void }) => JSX.Element;
  oppaiOracleModal?: (props: { isOpen: boolean; onClose: () => void }) => JSX.Element;
  ...
  addIpToMedia: (mediaSourceId: string, mediaId: string, ipId: string) => Promise<unknown>;
  removeIpFromMedia: (mediaSourceId: string, mediaId: string, ipId: string) => Promise<unknown>;
...
export function MediaSidebarContent(props: MediaSidebarContentProps) {
  ...
  const projects = createQuery<Project[]>(() =>
    props.projectsForMediaQueryOptions(props.media.mediaSourceId, props.media.id),
  );
  ...
  return (
    <MediaSidebar
      ...
      onCharacterAdd={async (characterId) => {
        await props.addCharacterToMedia(props.media.mediaSourceId, props.media.id, characterId);
      }}
      ...
      onDescriptionUpdate={async (description) => {
        await props.updateMediaDescription(props.media.mediaSourceId, props.media.id, description);
      }}
      onIpAdd={async (ipId) => {
        await props.addIpToMedia(props.media.mediaSourceId, props.media.id, ipId);
        props.onUpdate?.();
      }}
      ...
      onIpRemove={async (ipId) => {
        await props.removeIpFromMedia(props.media.mediaSourceId, props.media.id, ipId);
        props.onUpdate?.();
      }}
      onProjectAdd={async (projectId) => {
        await props.addProjectToMedia(props.media.mediaSourceId, props.media.id, projectId);
      ...
      onProjectRemove={async (projectId) => {
        await props.removeProjectFromMedia(props.media.mediaSourceId, props.media.id, projectId);
        await invalidateProjectsForMedia();
      }}
