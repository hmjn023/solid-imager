...
import { browserMeasurementSchema, type BrowserMeasurement } from "./measure-dev-startup-schema";
...
const rpcResponseFinishedMessage = "Dev startup measurement: matched RPC response finished";
...
function getProfile(): MeasurementProfile {
  ...
  throw new Error("--profile must be baseline, without-mkcert, or without-devtools");
}
...
function captureMilestone(text: string, startedAt: number, milestones: WorkerMilestones): void {
  milestones.output = `${milestones.output}${text}`.slice(-20_000);
  if (milestones.viteListeningAtMs === undefined && milestones.output.includes("➜  Local:")) {
...
async function waitForWorkerMilestones(milestones: WorkerMilestones): Promise<void> {
...
async function runMeasurement(): Promise<void> {
  ...
  const environment = getEnvironment(runtimeDir, routeTreePath, port, hmrPort, profile);
      ...
      JSON.stringify(
        {
          ...
          startupToStaticReadyMs: roundMilliseconds(browserMeasurement.staticReadyAtMs),
          ...
          staticReadyToFirstSsrHeadersMs: roundMilliseconds(
            browserMeasurement.firstSsrHeadersAtMs - browserMeasurement.staticReadyAtMs,
          ),
          staticReadyToFirstSsrHtmlMs: roundMilliseconds(
            browserMeasurement.firstSsrHtmlAtMs - browserMeasurement.staticReadyAtMs,
          ),
          ...
          firstSsrHtmlToInteractiveMs: roundMilliseconds(
            browserMeasurement.interactionReadyAtMs - browserMeasurement.firstSsrHtmlAtMs,
          ),
          firstNavigationTotalMs: roundMilliseconds(
            browserMeasurement.interactionReadyAtMs - browserMeasurement.firstNavigationStartedAtMs,
          ),
          firstRpcRequestToResponseMs: roundMilliseconds(
            browserMeasurement.firstRpcResponseAtMs - browserMeasurement.firstRpcRequestAtMs,
          ),
