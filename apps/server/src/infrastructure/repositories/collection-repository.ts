...
export const CollectionRepository: ICollectionRepository = {
	...
	async findById(id: string, tx?: Transaction): Promise<Collection | null> {
		...
		const result = await client.select().from(collections).where(eq(collections.id, id)).limit(1);
		return result[0] || null;
	},

	async create(collection: NewCollection, tx?: Transaction): Promise<Collection> {
		try {
			const client = (tx as unknown as TransactionClient) || db;
			const result = await client.insert(collections).values(collection).returning();
			return result[0];
		} catch (error: unknown) {
	...
	async update(id: string, updates: UpdateCollection, tx?: Transaction): Promise<Collection> {
		try {
			const client = (tx as unknown as TransactionClient) || db;
	...
	async delete(id: string, tx?: Transaction): Promise<void> {
		const client = (tx as unknown as TransactionClient) || db;
		const result = await client.delete(collections).where(eq(collections.id, id)).returning();
	...
	async addItem(collectionId: string, item: NewCollectionItem, tx?: Transaction): Promise<void> {
		try {
			const client = (tx as unknown as TransactionClient) || db;
	...
	async removeItem(collectionId: string, mediaId: string, tx?: Transaction): Promise<void> {
		const client = (tx as unknown as TransactionClient) || db;
		const result = await client
			.delete(mediaCollections)
			.where(
				and(eq(mediaCollections.collectionId, collectionId), eq(mediaCollections.mediaId, mediaId)),
			)
			.returning();
