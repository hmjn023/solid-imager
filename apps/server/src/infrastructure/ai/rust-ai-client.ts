...
export class RustAiClient implements IAiClient {
  ...
  private getExtensionFromBuffer(buffer: ArrayBuffer | Uint8Array): string {
    const bytes =
      buffer instanceof Uint8Array ? buffer.subarray(0, 4) : new Uint8Array(buffer).subarray(0, 4);
  ...
  private async callRemoteOrpcWithFile<T>(
    action: (client: ReturnType<typeof createRemoteOrpcClient>, file: File) => Promise<T>,
    ...
    const fileData = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
  ...
  async tagImage(imageBuffer: ArrayBuffer): Promise<TaggingResponse> {
    ...
    return this.withTempFile(imageBuffer, "rust-tag", (filePath) => this.tagImageByPath(filePath));
  }
  ...
  async extractCcipFeatureByPath(filePath: string): Promise<CcipFeatureResponse> {
    if (this.baseUrl) {
      const buffer = await Bun.file(filePath).bytes();
      const result = await this.callRemoteOrpcWithFile(
        (c, f) => c.ai.ccipFeature({ file: f }),
  ...
  async calculateCcipDistances(feature: number[], candidates: number[][]): Promise<number[]> {
    if (this.baseUrl) {
      if (!this.client) {
        throw new Error("Client is not initialized (baseUrl is empty)");
      }
