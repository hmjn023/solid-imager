...
import { getCcipVectorStatus, startCcipExtraction } from "~/infrastructure/api-clients/ai-api";
import {
  addCharacterToMedia,
  createCharacter,
  removeCharacterFromMedia,
} from "~/infrastructure/api-clients/characters-api";
import { addIpToMedia, createIp, removeIpFromMedia } from "~/infrastructure/api-clients/ips-api";
...
export function MediaSidebar(props: MediaSidebarProps) {
  ...
  const [isOppaiOracleModalOpen, setIsOppaiOracleModalOpen] = createSignal(false);

  const [isCharacterCropModalOpen, setIsCharacterCropModalOpen] = createSignal(false);
  ...
  const refreshCcipStatus = async () => {
    ...
    try {
      const result = await getCcipVectorStatus(props.media.mediaSourceId, props.media.id);
  ...
  const handleCcipExtraction = async () => {
    ...
    const isCurrentMedia = () =>
      props.media.id === currentMediaId && props.media.mediaSourceId === currentMediaSourceId;
    try {
  ...
  const handleAddProject = async (projectId: string) => {
    ...
    queryClient.invalidateQueries({
      queryKey: projectsForMediaQueryOptions(props.media.mediaSourceId, props.media.id).queryKey,
    });
    props.onUpdate?.();
  };
  ...
  const handleAddCharacter = async (characterId: string) => {
    ...
    const character = allCharacters.data?.find((c: { id: string }) => c.id === characterId);
                ...
                <li>
                  <div class="flex min-w-0 items-center gap-2">
                    ...
                    <Show when={author.accountId}>
                      <span class="break-all text-gray-500 text-xs">({author.accountId})</span>
                    </Show>
                  </div>
                </li>
              )}
