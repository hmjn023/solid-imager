...
const getAllCategoriesServer = async (): Promise<Category[]> => await categoryRepo.findAll();

const createCategoryServer = async (data: NewCategory): Promise<Category> =>
	await categoryRepo.create(data);

const getCategoryByIdServer = async (id: string): Promise<Category | undefined> => {
	const result = await categoryRepo.findById(id);
	return result ?? undefined;
};

const updateCategoryServer = async (id: string, data: UpdateCategory): Promise<Category> =>
