...
export class CharacterServiceImpl {
	...
	async addCharacterToMedia(mediaId: string, characterId: string): Promise<void> {
		return await this.transactionManager.transaction(async (tx: Transaction) => {
...
export const CharacterService = {
	getAllCharacters: async () => services.getCharacterService().getAllCharacters(),
	createCharacter: async (data: NewCharacter) =>
		services.getCharacterService().createCharacter(data),
	findByName: async (name: string) => services.getCharacterService().findByName(name),
	getCharacterDetails: async (id: string) => services.getCharacterService().getCharacterDetails(id),
	updateCharacter: async (id: string, data: UpdateCharacter) =>
		services.getCharacterService().updateCharacter(id, data),
	deleteCharacter: async (id: string) => services.getCharacterService().deleteCharacter(id),
	...
	removeCharacterFromMedia: async (mediaId: string, characterId: string) =>
		services.getCharacterService().removeCharacterFromMedia(mediaId, characterId),
};
