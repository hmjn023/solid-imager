...
function makeMedia(id: string, mediaSourceId = "source-1", filePath = `/media/${id}.png`) {
...
describe("MaintenanceService", () => {
	...
	describe("performStartupChecks", () => {
		it("should run both queueMissingMetadata and queueMissingThumbnails without throwing", async () => {
			...
			expect(mockMediaRepo.findIdsWithMissingGenerationInfo).toHaveBeenCalledOnce();
	...
	describe("queueMissingMetadata", () => {
		...
		it("should create a job for each media that is missing generation info", async () => {
			...
			mockMediaRepo.findIdsWithMissingGenerationInfo.mockResolvedValue([media1, media2]);
			mockMediaRepo.findAllMediaIndices.mockResolvedValue([]);

			mockSourceRepo.findById.mockResolvedValue(makeLocalSource("source-1", "/local/images"));
		...
		it("should process items in chunks of 50", async () => {
			...
			const items = Array.from({ length: TOTAL_ITEMS }, (_, i) => makeMedia(`media-${i}`));
		...
		it("should skip media whose source is not a local source", async () => {
			...
			mockMediaRepo.findIdsWithMissingGenerationInfo.mockResolvedValue([media1]);
			mockMediaRepo.findAllMediaIndices.mockResolvedValue([]);
	...
	describe("queueMissingThumbnails", () => {
		it("should NOT create any jobs when all thumbnails exist", async () => {
			...
			(fs.readdir as unknown as Mock).mockResolvedValue(["media-1.webp"] as any);
		...
		it("should stop fetching batches when batch size equals BATCH_SIZE (simulate full page)", async () => {
			...
			const batch1 = Array.from({ length: BATCH_SIZE }, (_, i) => makeMedia(`m-${i}`));
			...
			(fs.readdir as unknown as Mock).mockResolvedValue(batch1.map((m) => `${m.id}.webp`) as any);
			...
			expect(mockMediaRepo.findAllMediaIndices).toHaveBeenNthCalledWith(1, undefined, {
				limit: 1000,
				offset: 0,
			});
			expect(mockMediaRepo.findAllMediaIndices).toHaveBeenNthCalledWith(2, undefined, {
				limit: 1000,
				offset: 1000,
			});
			expect(mockJobRepo.createIfUnique).not.toHaveBeenCalled();
