...
export type PgliteConnectionDetails = z.infer<typeof PgliteConnectionDetailsSchema>;
...
export function loadDatabaseConfig(configPath: string = process.cwd()): DatabaseConfig {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ENOENT") {
			throw new Error(`Database configuration file not found: ${CONFIG_FILE_NAME}.`);
		}
