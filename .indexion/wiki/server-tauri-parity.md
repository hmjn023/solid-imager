...
主要 route（search / sources / manager / config / source-media）は `packages/ui/src/screens/*` と `packages/ui/src/hooks/use-*-page.ts` を利用する形へ前進した。#301 / #307 の merged により search / manager は thin wrapper 化済み。#302 / #306 は PR #313 で実装済みだが未 merge のため、現時点では「PR中」として扱う。

残る route 側差分は、transport / filesystem / IPC / platform 固有 render adapter に閉じる方向へ縮退している。ただし #313 merge 前の base では `sources/index.tsx` と `source-media-page.tsx` にまだ旧記述が残るため、評価は PR #313 反映後の実態を前提にした暫定値。
...
ページ状態管理 hook（`use-search-page` / `use-sources-page` / `use-source-media-page` / `use-manager-page`）は `packages/ui` へ共有化され、双方から利用されている。#303 で event payload parser は `packages/core/src/utils/event-parsers.ts` に抽出済み。#302 / PR #313 で sources 一覧の event schema parse / relevance filter / state update は `packages/ui/src/hooks/use-sources-events.ts` に集約済み。残る差分は `use-media-source-events.ts` の app 別 transport wrapper と、source-media 詳細ページの platform event payload 差分。
...
| `use-sources-events.ts`             | `packages/ui` の shared hook                                  | `packages/ui` の shared hook                                                    | transport callback のみ                          | #302 / PR #313。`all-jobs-completed` / `watcher-error` の schema parse、sourceId relevance filter、query invalidation、toast を shared 化       |
...
| `media/thumbnail-image.tsx`               | thin wrapper           | `packages/ui/src/thumbnail-image.tsx` と `packages/ui/src/thumbnail-source.ts` を共有。HTTP retry/cache key、local thumbnail/original fallback、object URL cleanup、thumbnail-ready 再取得は shared 化済み。server/tauri 側差分は URL builder、filesystem、`convertFileSrc`、path join、event subscription adapter に限定 |
...
`packages/db` に repository factory はほぼ集約され、author / category / character / collection / ip / media / preset / project / source / tag / user / job は server / tauri とも shared repository contract を使う構成へ前進した。app 固有 repository は app-config や一部 job/bootstrap 周辺に残るが、CRUD 系 repository parity の穴はほぼ解消。#322 の再監査では、残差分は shared 化候補ではなく app adapter / bootstrap 層として扱う。
...
| `media-search` (util)      | `packages/db/src/repositories/media-search.ts`          | 旧 `media-repository-utils.ts` 相当の検索ロジック。server 側 legacy util は #322 で統合・削除済み | 同上                                                      |
...
| リポジトリ                 | 分類                  | 理由                                                                                                                          |
| -------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `job-repository.ts`        | 完了済み              | server / tauri とも `packages/db/src/repositories/job-repository.ts` factory を使う薄い wrapper                               |
| `tauri-job-repository.ts`  | platform I/O 例外     | shared job repository に `processMedia` payload 変換と Tauri queue 用 helper を重ねる thin wrapper。DB query logic は shared 側 |
| `app-config-repository.ts` | platform I/O 例外     | `ConfigServiceImpl` の store adapter。Tauri は PGlite `appConfig` table、server は file/env store を使うため persistence I/O 差分 |
| `media-repository-utils.ts` | 完了済み              | server 側 legacy util は `packages/db/src/repositories/media-search.ts` へ統合し、ファイルは削除済み（#322）             |

## Services（対応度: ~88%）

`packages/application` の追加自体は前進し、CRUD 系はほぼ shared 化完了。source / media / backup など主機能も shared contract を使う形へ前進。tauri 側 `media-service.ts` は `createMediaService` を使う thin adapter に縮退し、`updateMedia`/`copyMedia`/`moveMedia`/`deleteMedia` の public contract を server 側に揃えた。`source-backup-service.ts` も `_` プレフィックス付き内部メソッドの公開を削除し、`createDump`/`importSourceZip` の platform I/O 差分のみを adapter として残す。#322 の再監査では、追加で即時 shared 化すべき service logic は見つかっていない。
...
厳格基準では、`apps/tauri/src/infrastructure/local-api/services/source-service.ts` は shared `createSourceService` と `createSourceSyncRuntime` を使う thin adapter へ前進。list/get/testConnection/getStatus に加えて、local source の scan / probe / upsert / media event publish / process-media enqueue / delete reconciliation / watch event payload parse は `packages/application/src/services/source-sync-runtime.ts` に集約済み。Tauri 側に残る差分は Rust watcher ingress、Tauri event transport、Rust `probe_media` command、local filesystem adapter、local source validation、source CRUD wrapper。`media-service.ts` は shared `packages/application/src/services/media-service.ts` を使う thin adapter に縮退。`contextMetadataUpdater` / `afterMediaRegistered` / `extractAndUpdateMetadata` は shared 化済み。`source-backup-service.ts` も shared `createBackupService` を使い、`_` プレフィックス付き内部メソッドの公開を削除。upload collision resolution algorithm も `packages/application/src/services/media-upload-utils.ts` に shared 化し、server / tauri 両側が `resolveUploadTargetPath` を利用。server 側 `server-media-storage.ts` と `download-jobs.ts` の独自実装を削除。残る未 shared 部分は zip 処理など platform 固有 I/O 層。
...
| `source-service`          | `packages/application/src/services/source-service.ts`, `packages/application/src/services/source-sync-runtime.ts` | server は shared service を利用。tauri も `createSourceService` と `createSourceSyncRuntime` を使い、CRUD wrapper / watcher ingress / fs / Rust command / event transport だけを adapter に残す |
...
| サービス                      | 分類              | 対応するtauri実装 / 理由                                                                                                      |
| ----------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `analytics-service.ts`        | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
| `backup-service.ts`           | platform I/O 例外 | `source-backup-service.ts`（tauri）。DB dump/restore は shared、zip stream / bytes / fs adapter が app 固有                   |
| `bulk-operation-service.ts`   | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
| `category-service.ts`         | 完了済み          | `TauriCategoryService`（local-api/services/）。shared `createCategoryService` 利用                                            |
| `collection-service.ts`       | 完了済み          | `TauriCollectionService`（local-api/services/）。shared `createCollectionService` 利用                                        |
| `data-migration-service.ts`   | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
| `directory-service.ts`        | server-only 例外  | server storage driver（local / sftp / s3）経由の directory 操作。Tauri remote source は parity 対象外                         |
| `directory-sync-service.ts`   | platform I/O 例外 | tauri は `source-service.ts` + Rust watcher / fs / probe。reconciliation は `source-sync-runtime` / `watcher-runtime` で共有  |
| `filter-preset-service.ts`    | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
| `integration-service.ts`      | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
| `job-dispatch-service.ts`     | platform I/O 例外 | tauri は `tauri-job-queue.ts` + Rust backend 統合。canonical job type / dispatcher は shared 化済み                            |
| `media-processing-job.ts`     | 完了済み          | tauri `process-media-job.ts` は `@solid-imager/application/services/media-processing-job` の型定義 re-export                  |
| `media-processing-service.ts` | platform I/O 例外 | tauri は Rust commands + `tauri-job-queue.ts`。process runner / metadata persistence / job payload は shared 化済み            |
| `media-source-service.ts`     | 完了済み          | tauri `source-service.ts` は shared `createSourceService` と `createSourceSyncRuntime` を利用する adapter                      |
| `search-service.ts`           | 完了済み          | search query logic は `packages/db/src/repositories/media-search.ts` と shared query options 側。tauri は API client 経由      |
| `server-config-service.ts`    | platform I/O 例外 | server は file/env `ConfigStore`、tauri は PGlite store。両方とも shared `ConfigServiceImpl` を利用                            |
| `tagging-service.ts`          | 完了済み          | `packages/application/src/services/ai-tagging-service.ts` へ shared 化。server / tauri とも shared contract を利用（#322） |
| `thumbnail-service.ts`        | platform I/O 例外 | server HTTP URL / cache fs と Tauri Rust IPC / local file URL の差分。rendering helper は `thumbnail-source.ts` で shared 済み |
| `user-service.ts`             | 完了済み          | `TauriUserService`（local-api/services/）。shared `createUserService` 利用                                                    |
| `workflow-service.ts`         | 完了済み          | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub は shared 化済み                               |
...
| 領域                 | 分類              | 判断                                                                                                                  |
| -------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------- |
| job repository       | 完了済み          | shared `createJobRepository` を両 app が使用。Tauri 固有 helper は processMedia payload 変換のみ                      |
| watcher ingress      | platform I/O 例外 | server は chokidar、tauri は Rust watcher。delete/change reconciliation は shared runtime へ移動済み                  |
| event transport      | platform I/O 例外 | server は SSE、tauri は Tauri event bus。event contract / payload helper は shared 化済み                             |
| thumbnail generation | platform I/O 例外 | server は Node image/fs adapter、tauri は Rust IPC batch。processMedia runner と thumbnail-ready event は shared 契約 |
| download runner      | 完了済み          | runner / URL helper / collision resolution は shared 化済み。残る HTTP / fs write / queue wake は adapter             |
...
| **低**     | **Repositories**                                | 主要 CRUD は shared factory 化済み。残る app-config / tauri-job wrapper は platform I/O / queue adapter                                                                                  | ほぼ完了 |
...
| **低**     | **Services**                                    | CRUD / media / backup / ai tagging は shared 化済み。残るは platform 固有 I/O（zip stream / HTTP URL / storage driver）と shared stub の legacy wrapper                                 | ほぼ完了 |
...
1. **Components / `media-sidebar-content.tsx`** — #298 / PR #311。`packages/ui/src/media-sidebar.tsx` へ抽出済み
2. **Components / `source-form-modal-content.tsx`** — #299 / PR #311。`packages/ui/src/source-form-modal.tsx` へ抽出済み
3. **Components / `upload-media-modal-content.tsx`** — #300 / PR #311。`packages/ui/src/upload-media-modal.tsx` へ抽出済み
4. **Routes / `search.tsx`** — #301 / PR #312。filter query wiring、mobile dialog、refresh 戦略を shared 化済み
5. **Hooks / `use-batch-job-events.ts` parser** — #303 / PR #310。parser utility を `packages/core/src/utils/event-parsers.ts` へ抽出済み
6. **Components / `thumbnail-image.tsx`** — #304 / PR #310。`ThumbnailSource` adapter pattern で shared 化済み
7. **Components / `media-viewer.tsx`** — #305 / PR #310。`MediaSource` adapter pattern で shared 化済み
...
- services: CRUD / media / backup / ai tagging は shared 化済み。残るは platform 固有 I/O（zip stream / HTTP URL / storage driver）と shared stub の legacy wrapper。実態評価 ~88% は妥当
...
- **Components / Media Sidebar (#298, PR #311)**: `packages/ui/src/media-sidebar.tsx` を新設。server / tauri の `media-sidebar-content.tsx` は data/action adapter 注入へ縮退
- **Components / Source Form Modal (#299, PR #311)**: `packages/ui/src/source-form-modal.tsx` を新設。フォーム本体を shared 化し、validation / submit adapter の差分だけ app 側に残す
- **Components / Upload Media Modal (#300, PR #311)**: `packages/ui/src/upload-media-modal.tsx` を新設。ドラッグ&ドロップ、ファイル選択、衝突解決表示を shared 化
- **Routes / Search (#301, PR #312)**: `SearchScreen` / `useSearchPage` に filter query wiring、mobile filter dialog、refresh strategy を集約。route 側は query/API/event/render adapter 注入へ縮退
- **Routes / Sources (#302, PR #313)**: `packages/ui/src/hooks/use-sources-events.ts` を新設。sources 一覧 event の schema parse、sourceId relevance filter、query invalidation、watcher-error toast を shared 化
- **Hooks / Batch Job Parser (#303, PR #310)**: `packages/core/src/utils/event-parsers.ts` を新設し、`SafeParseSchema` / `parseJsonEventPayload` / `parseEventPayload` を共有化
- **Components / Thumbnail Image (#304, PR #310)**: `packages/ui/src/thumbnail-image.tsx` を新設し、`ThumbnailSource` adapter pattern で shared 化
- **Components / Media Viewer (#305, PR #310)**: `packages/ui/src/media-viewer.tsx` を新設し、`MediaSource` adapter pattern で shared 化
- **Routes / Source Media (#306, PR #313)**: `SourceMediaScreen` 側へ upload / move-copy modal JSX assembly を移動。route 側は transport / platform adapter 注入中心へ縮退
- **Routes / Manager (#307, PR #312)**: `useManagerPage` に query/prefetch/action/event wiring を集約し、route 側は API/event/render adapter 注入へ縮退
- **Server Legacy Event Service (#308, PR #310)**: legacy `event-service.ts` を削除。`SseManager` / Tauri event bus を transport adapter として扱う方針へ統一
- **Components / Media Card & Grid Items (#309, PR #311)**: `packages/ui/src/media-card-item.tsx` / `packages/ui/src/media-grid-item.tsx` を新設。差分は link adapter、thumbnail renderer、`sourceRootPath` 注入に集約
...
| `directory-sync-service`   | server のみ  | tauri 側の同等処理は `packages/application/src/services/source-sync-runtime.ts` へ移動済み。残る差分は watcher ingress、filesystem adapter、probe/storage adapter、SSE vs Tauri event transport、server 側 `DirectorySyncService` の既存 public wrapper |
...
## 残差分監査（2026-05-01 — #322）

| 分類              | 項目                                                                                          | 判断                                                                                                                  |
| ----------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| 完了済み          | `media-repository-utils.ts`                                                                    | server 側 legacy util は `packages/db/src/repositories/media-search.ts` へ統合・削除済み（#322）                         |
| 完了済み          | `tagging-service.ts` / `ai-service.ts` の AI tagging orchestration                              | `packages/application/src/services/ai-tagging-service.ts` へ shared 化。path/buffer fallback、cache response reconstruction、CCIP path 判定を shared contract に集約（#322） |
| 要監視            | `ServerMediaStorage` / `tauriMediaStorage`                                                     | `IMediaStorage` adapter として説明可能だが、scan / metadata / safe path / copy の重複が再増殖しないか追跡する               |
| platform I/O 例外 | zip stream / byte array / local fs adapter、thumbnail I/O、SSE / Tauri event、Rust IPC         | shared contract は存在し、差分は transport / filesystem / command invocation に閉じる                                |
| platform I/O 例外 | watcher ingress（chokidar / Rust watcher）                                                     | change/delete reconciliation と source sync runtime は shared 化済み。ingress は OS / runtime integration             |
| platform I/O 例外 | `app-config-repository.ts`、`tauri-job-repository.ts`                                          | persistence store / queue helper の adapter。DB query と service logic は shared factory / shared service 側          |
| server-only 例外  | `directory-service.ts`                                                                         | server storage driver（local / sftp / s3）向け。Tauri remote source は当面 parity 対象外                              |
| 完了済み          | analytics / bulk-operation / data-migration / filter-preset / integration / workflow services | `packages/application/src/services/stub-services.ts` の re-export。未実装 stub を app ごとに二重実装していない        |
| 完了済み          | CRUD / media / source / backup DB logic / tagging persistence / job worker / runners           | `packages/application` / `packages/db` に共有化済み。app 側は executor、storage、transport、queue wake の注入が中心 |

#322 の初回監査で残っていた shared 化候補（server 側 legacy search util、AI tagging orchestration）は 2026-05-04 に完了。`media-repository-utils.ts` は `packages/db/src/repositories/media-search.ts` へ統合・削除、`tagging-service.ts` の AI path/buffer fallback・cache response reconstruction・CCIP path 判定は `packages/application/src/services/ai-tagging-service.ts` へ集約。media storage adapter は引き続き platform I/O 例外に留め、重複増殖を監視する。
...
- **Services / Source Sync Runtime**: `packages/application/src/services/source-sync-runtime.ts` を新設。Tauri `source-service.ts` に残っていた watched path dedupe、recursive scan、supported media 判定、probe → media upsert input、`media-added`/`media-changed`/`media-deleted` publish、process-media enqueue、file/directory delete reconciliation、watch event payload parser を shared runtime 化。app 側差分は watcher ingress / filesystem / probe command / event transport / queue adapter に再分類
