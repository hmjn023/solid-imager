...
- Web と Tauri の global search、source search は `searchHistoryQuerySchema` で `search` query を検証する。
- 共通の復元処理は `packages/ui/src/hooks/use-search-history-persistence.ts` に置き、route 側は API client と共通設定を渡すだけにする。
