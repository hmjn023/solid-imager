...
*   **`packages/ui`**: プロジェクト全体で共有されるUIコンポーネントライブラリ。Kobalte や Tailwind CSS をベースにしたデザインシステムの実装。
*   **`apps/server`**: サーバーサイドの実装。SolidStart (Vinxi), Drizzle ORM, Node.js API を使用し、`packages/core` で定義されたインターフェースを実装します。
*   **`apps/cli`**: コマンドラインツール（CLI）および Model Context Protocol (MCP) サーバー。サーバーの API や機能をターミナルから操作・管理するためのインターフェースを提供します。
...
packages/ui/src/         # [共有UI]
├── components/          # 汎用UIコンポーネント群 (Button, Input, Dialog 等)
...
apps/cli/src/
├── commands/            # CLIコマンドの定義と実行ロジック
└── mcp/                 # MCPサーバーの公開リソースやプロンプトの定義
```
