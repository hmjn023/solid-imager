...
*   **`DownloadItem` (for Download/Xtracter)**:
    ...
    *   *変更点*: 現状の `DownloadItem` が `MediaDumpItem` を継承しているために不要な物理ファイル情報を持っている問題を、基底スキーマを分離することで解消する。
...
*   **手動再実行対応**: 失敗したステップは、UIから個別に再実行できるようにoRPCプロシージャを用意する（例: `media.regenerateThumbnail`）。
