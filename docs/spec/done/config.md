...
**※データベース設定 (`database`) について:**
DB接続情報（ホスト、ユーザー、パスワード等）や接続タイプ（PGlite/Postgres）は、セキュリティおよびマイグレーションツール (`drizzle-kit`) との互換性を考慮し、引き続き **環境変数 (`.env`)** で管理する。

### 2. 各セクションの詳細

#### 2.1 Job Processing (`jobs`)
...
#### 2.4 Media Configuration (`media`)
メディアファイルの取り扱いに関する詳細設定。

##### 2.4.1 Supported Extensions (`supportedExtensions`)
...
##### 2.4.2 Tag Extraction (`tagExtraction`)
...
1. **初期化**:
    ...
    - **自動生成**: ファイルが存在しない場合は、ハードコードされた**デフォルト値を持つ `config.json` を新規作成**し、それを使用して起動する。
2. **バリデーション**:
    - 読み込み時に JSON Schema (zod等) を用いて値を検証する。不正な値がある場合はデフォルト値にフォールバックするか、起動を停止する。
3. **環境変数オーバーライド**:
    - `config.json` の値よりも**環境変数を優先**して適用する仕組みを導入する。
    - 書式: `CONFIG_<SECTION>_<KEY>` (アンダースコア区切り)
    - 例: `CONFIG_AI_BASE_URL=http://ai-service:8000` は `ai.baseUrl` を上書きする。
4. **動的更新**:
