...
## 8. UI設計方針 (検索モーダル/ドロワー)

ユーザーの習熟度に合わせ、「簡易モード」と「プロモード」の2つのUIモードを提供する。

### 簡易モード (Simple Mode)

現在と同様の直感的なUI。複雑な論理演算を意識させず、入力した条件は全て **AND** で結合される。
...
*   **入力項目**:
    ...
    *   IP選択 (複数選択可)
    *   スター評価 (以上/以下)
*   **内部ロジック**: 各フィールドの入力値を `type: "group", operator: "and"` で束ねてAPIに送信する。
    *   例: タグ `A`, `B` が選択された場合 -> `tag=A AND tag=B`

### プロモード (Pro Mode)

再帰的なクエリ構造を視覚的に構築できる高機能UI。SQLのWHERE句ビルダーに近い操作感を提供する。

*   **UI構造**: ツリー構造のエディタ
*   **ルート要素**: 常にルートとなる `Group` が存在し、デフォルトは `AND` 結合。
*   **操作フロー**:
    1.  **条件追加**: 「＋条件を追加」ボタンで、現在のグループに新しい条件行を追加。
        *   セレクトボックス: 対象フィールド (`tag`, `width`, `fileName` 等)
        *   セレクトボックス: 演算子 (`equals`, `contains`, `gt` 等)
        *   インプット: 値
    2.  **グループ追加**: 「＋グループを追加」ボタンで、ネストされた条件グループを追加。
        *   インデントされた領域が表示される。
        *   グループヘッダーで結合条件 (`AND` / `OR` / `NOT`) を切り替え可能。
    3.  **削除**: 各行・各グループの右端にある「削除」ボタンで撤去。
*   **バリデーション**: UI上でネスト深度制限(10階層)を超えないように制御する。
...
*   **入力補完 (Autocomplete)**: `tag`, `character`, `ip`, `author`, `project` などのフィールド入力時は、既存の実装と同様にサーバーサイドからの候補取得による入力補完を提供する。これは「簡易モード」「プロモード」双方で必須とする。