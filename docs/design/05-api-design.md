...
#### メディア管理
```
GET    /api/sources/:sourceId/:mediaId                  # 特定のメディアのオリジナル画像を配信します。(sourceId: UUID, mediaId: UUID)
...
POST   /api/sources/:sourceId/:mediaId/tags             # 特定のメディアにタグを追加します。(sourceId: UUID, mediaId: UUID, body: { tagId: number, source?: string })
...
POST   /api/sources/:sourceId/:mediaId/characters       # 特定のメディアにキャラクターを追加します。(sourceId: UUID, mediaId: UUID, body: { characterId: number, source?: string })
...
POST   /api/sources/:sourceId/:mediaId/ips              # 特定のメディアにIPを追加します。(sourceId: UUID, mediaId: UUID, body: { ipId: number, source?: string })
...
##### `MediaSourceInfo`
メディアソースの作成・更新時に使用されるデータ構造です。`POST /api/sources`, `PUT /api/sources/:sourceId` のリクエストボディとして使用されます。
```typescript
...
##### `MediaUpdateData`
メディア情報の更新時に使用されます。`PUT /api/sources/:sourceId/:mediaId` のリクエストボディです。
```typescript
...
##### `UploadRequest`
メディアのアップロード時に使用されます。`POST /api/sources/:sourceId/upload` で `multipart/form-data` として送信されます。
```typescript
...
ディレクトリ操作 (`POST /api/sources/:sourceId/directories`, `PUT /api/sources/:sourceId/directories`, `DELETE /api/sources/:sourceId/directories`) のリクエストボディです。
```typescript
