...
このリポジトリは Vite、Vitest、Biome を使用します。依存関係の更新後や作業開始時は AGENTS.md のチェックリストを優先してください。
...
通常のコード修正では開発サーバーを起動しません。UI 実装やブラウザ検証が必要な場合のみ、既存ポートを確認して `bun run dev` など適切なコマンドを使います。

### コード品質

コミット前には Biome / TypeScript のチェックを実行してください。
